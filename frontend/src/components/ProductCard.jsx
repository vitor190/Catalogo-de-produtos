...
export function ProductCard({ produto, aoExcluir, aoEditar }) {
  ...
  return (
    <div className="bg-white p-[16px] rounded-[12px] border border-[#E5E7EB] shadow-sm hover:shadow-md transition-all duration-300 flex flex-col h-full">
      ...
      <div className="w-full h-40 bg-gray-100 rounded-[8px] mb-4 overflow-hidden">
        <img 
      ...
      <h3 className="text-[#1F2937] text-[16px] md:text-[18px] font-semibold mt-1">{produto.nome}</h3>
      <p className="text-[#1F2937] text-[20px] md:text-[22px] font-bold my-2">
      ...
      <div className="flex gap-2 mt-auto">
        <button 
          onClick={aoEditar} 
          className="flex-1 border border-[#3B82F6] text-[#3B82F6] hover:bg-[#3B82F6] hover:text-white py-[12px] rounded-[8px] font-semibold transition-all"
        >
          Editar
        </button>
        <button 
